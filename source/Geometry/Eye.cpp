...
namespace Geometry
{
	...
	template <typename NumericT>
	Eye<NumericT> Eye<NumericT>::Transformation::convert_from_viewport_space_to_object_space (const Box<2, NumericT> & viewport, const Vector<2, NumericT> & point) const
	{
	...
	Eye<NumericT> Eye<NumericT>::Transformation::convert_from_normalized_space_to_object_space (Vector<3, NumericT> normalized_point) const
	{
		...
		auto p1 = (inverse_view_matrix * (inverse_projection_matrix * normalized_point.append(1.0)));
		...
		auto p2 = (inverse_view_matrix * (inverse_projection_matrix * normalized_point.append(1.0)));
	...
	Matrix<4, 4, NumericT> Eye<NumericT>::look_at(const Vector<3, NumericT> & origin, const Vector<3, NumericT> & direction, const Vector<3, NumericT> & up)
	{
		...
		Matrix<4, 4, NumericT> result = Transforms::translate(-origin);
